package com.fiap.auth_service._webApi.controller.errorHandler;
...
class GlobalHandlerExceptionTest {
    ...
    void shouldHandlePropertyValueException() {
        PropertyValueException ex = new PropertyValueException("msg", "Entity", "propName");

        ResponseEntity<String> response = handler.handlePropertyValueException(ex);

        assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
        assertEquals("O campo 'propName' é obrigatório.", response.getBody());
    }
}